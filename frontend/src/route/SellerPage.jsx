...
const SellerDashboard = () => {
  ...
  //useEffect to get the ratings of the seller
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.status === 404) {
        alert("Product is already in your shop.");
      } else {
  ...
  const ProductCard = ({ product }) => (
    <div className="productCard fadeIn">
      ...
      <div className="productImageContainer">
        ...
        />
        <span
          className={`productStatus ${product.status === "Active" ? "statusActive" : "statusInactive"
            }`}
        >
          {" "}
          {/* Changed class name */}
